...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // API returns 'trains' at root level, not 'trainData'
            const {
                accessToken,
                refreshToken,
                admin,
                ...
                sectionId,
                otherStations,
                trains  // ← Correct field name from API
            } = response;
            ...
            if (admin.isSectionAdmin && sectionData) {
                ...
                // Pass trains as trainData to maintain Redux slice compatibility
                dispatch(setSectionData({
                    sectionData,
                    sectionId: admin.sectionId || sectionData.section_id,
                    trainData: trains || []  // ← Map 'trains' to 'trainData'
                }));
            } else if (stationData) {
            ...
            // Run optimization in background after storing data
            const optimizationSectionData = admin.isSectionAdmin ? sectionData : (sectionData || stationData);
            if (optimizationSectionData && trains && trains.length > 0) {
                dispatch(setOptimizationLoading());
                optimizeScheduleApi(optimizationSectionData, trains)
                    .then(result => {
                        dispatch(setOptimizationResults(result));
                        console.log('Optimization completed:', result.message);
                    })
                    .catch(err => {
